function init() {
    ...
    if(urlParams.get('tag') === null || urlParams.get('tag') === '') {
        compensation_404();
        return;
    }
    try {
...
function search(tag) {
    //tag = validateQuery(tag);
    fetch('https://nhentai-api.onrender.com/https://fuxnxx.com/tags/' + tag + '.html')
    .then(response => {
        if (!response.ok) {
            compensation_404();
            return undefined;
    ...
    .then(html => {
        ...
        document.querySelector('h3').innerHTML += tag + ':';
        ...
        if (doc.querySelector("body > div.thumbnail > div.toptitle > h1").innerHTML.includes("Can't Found")) {
            document.querySelector('h3').innerHTML = "Couldn't find videos for: " + tag + '.<br>Default selection shown below.';
        }
